...
\Comdiag{F(a)  \ar[r, "\alpha_a"] \ar[d, "F(h)"] \& G(a)  \ar[d, "G(h)"]\\
...
\subsection{Universal properties}\ref{universalsec}
...
Groups seem awkward to me because they reveal how the $X\times Y \to X$ mapping isn't
...
But then, recall that even identity arrows are item specific: the arrow $3\to 3$ and $4\to
4$ are distinct parts of the category of $(\Re, \cdot)$, even though we have a convenient
...
category. Here, there exists a unique mapping $A\to T$, for any $A$. Generally, this
...
is $\max(A)$; else if $A$ is inifinite the terminal set doesn't exist, it seems to me.
...
\section{The injective retraction epic} These are some themes that seemed to repeat at
...
First, if two elements $a_1$ and $a_2$ have the same target $b_1$, reversing the arrows
...
\Comdiag{ \& B\ar[dr,dashrightarrow,"r"]\\
...
\Comdiag{ \& B\ar[dr,dashrightarrow,"g"]\\
A\ar[ur,"f"]\ar[rr,"h"]\&\&C
}
